...
interface AppContextValue {
  ...
  saveNow: () => void;
  /** Passwort für künftige Speicherungen ändern (null = unverschlüsselt). */
  setFilePassword: (password: string | null) => void;
}
...
export function AppProvider({ session, children }: { session: Session; children: ReactNode }) {
  ...
  const passwordRef = useRef(session.password);
  const isFirstRender = useRef(true);

  const writeNow = useCallback(
    async (snapshot: AppData) => {
      try {
        const text = await serializeFile(snapshot, passwordRef.current);
