...
- **Bewusste Abweichung (Sicherheit):** Die Sicherheitsfrage der Alt-App entfällt — sie setzte nur das Login zurück, konnte die AES-verschlüsselten Daten aber nicht retten (Scheinsicherheit). Stattdessen: Key-Wrapping — ein zufälliger Datenschlüssel verschlüsselt die Datei und wird zweimal eingewickelt (Passwort + generierter Wiederherstellungsschlüssel XXXX-XXXX-XXXX-XXXX). Beide öffnen die Datei; der Recovery-Key wird beim Einrichten einmalig angezeigt. Ein separater Login-Screen entfällt ebenfalls: Die Passwortabfrage beim Öffnen der verschlüsselten Datei IST das Login.
